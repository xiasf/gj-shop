...
class UserController extends MobileBaseController
{
    ...
    public function points()
    {
        ...
        if ($type == 'recharge') {
            ...
            $count       = M('account_log')->where("user_id=" . $this->user_id . " and exchange!=0 ")->count();
            $Page        = new Page($count, 16);
            $account_log = M('account_log')->where("user_id=" . $this->user_id . " and exchange!=0 ")->order('log_id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        } else {
...
// 上传错误提示错误信息
    ...
    public function invitation()
    {
        ...
        $exchange = (float) M('invitation')->where(['leader_uid' => $this->user_id, 'status' => 1])->sum('exchange');
        $Page     = new Page($count, 16);
