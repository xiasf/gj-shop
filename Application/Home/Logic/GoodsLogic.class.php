...
class GoodsLogic extends RelationModel
{
    ...
    public function getGoodsIdByAttr($attr)
    {
        ...
        $c     = count($attr_id) - 1;
        if ($c > 0) {
            $arr = M('goods_attr')->where($where)->group('goods_id')->having("count(goods_id) > $c")->getField("goods_id", true);
        }
        //select * from   `tp_goods_attr` where attr_id in(59,80) and attr_value in('直板','翻盖','蓝牙4.0') group by goods_id having count(goods_id) > 1
        else {
            $arr = M('goods_attr')->where($where)->getField("goods_id", true);
        }
        // 如果只有一个条件不再进行分组查询
