...
function get_thum_images($src, $width, $height)
{
    ...
    $a = $image->thumb($width, $height, 3)->save('.' . $path . $thumb, null, 100); //按照原图的比例生成一个最大为$width*$height的缩略图并保存
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $exchange = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '', $proportion = 1)
{
    ...
    foreach ($order_goods as $key => $val) {
        ...
        if ($exchange && $on_exchange) {
            ...
            if ($exchange >= $on_exchange) {
                ...
                $on_pay_exchange = $exchange;
            }
            $exchange -= $on_pay_exchange;
    ...
    if ($order_amount > 0) {
        $pay_exchange   = ($pay_exchange / tpCache('shopping.exchange_rate')); // 兑币支付
    ...
    if ($order_amount > 0) {
        ...
        // 积分抵扣的金额怎么能出现小数，那就成BUG了，所以要直接取整，不要小数位（这对用户来说不会损失什么，相当于如果50积分等于1元的话，那么当积分抵扣1.11元时，会出现使用55.5个积分的情况，所以要处理一下，以保证积分是整数，这样会出现这样的情况，当系统计算得到积分需要抵扣1.11元时，即使用户输入56个积分来抵扣，那么也不会出现扣用户55.5或者56个积分的情况的，不会多扣积分，相当于强制让用户输入了55，使用55个积分来抵扣抵扣1.1元，在保证安全的同时最大限度的满足用户使用出去积分）
        // tpCache('shopping.point_rate')是1~100的整数
        $pay_points_num = $pay_points * tpCache('shopping.point_rate');
        if (!is_int($pay_points_num)) {
            $pay_points_num = (int) $pay_points_num;
            $pay_points = $pay_points_num / tpCache('shopping.point_rate');
        }
...
{
    $arr = ["餐饮", "电影", "酒店", "休闲娱乐", "养生保健", "酒水", "美容美发", "金融", "旅游", "宾馆", "服装", "鞋包", "汽车", "房产", "保险", "咨询", "建材", "装修", "医疗", "家政", "文化", "教育", "运动", "超市", "特产", '家居', "其他"];
