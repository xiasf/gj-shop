...
class PluginController extends BaseController
{
    ...
    private function scanPlugin()
    {
        ...
        // 各类插件的所有插件
        $plugin_list = [
            'payment'  => $this->dirscan(C('PAYMENT_PLUGIN_PATH')),
            'login'    => $this->dirscan(C('LOGIN_PLUGIN_PATH')),
            'shipping' => $this->dirscan(C('SHIPPING_PLUGIN_PATH')),
            'function' => $this->dirscan(C('FUNCTION_PLUGIN_PATH')),
        ];
    ...
    private function dirscan($dir)
    {
        ...
        if (false != ($handle = opendir($dir))) {
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                // 其实就是想得到某类插件下面的插件列表（获取目录就可以了）
                // 可以看出重构之后代码更加简洁，语义更加明确，程序可读性更高
                if($file == '.' || $file == '..') {
                    continue;
                }
                if (is_dir($dir . '/' . $file)) {
